...
var REG_CONF = {
    REG_SRC: /src\=[\"\']([\w\.\/\'\"\s\-]+|[a-zA-z]+:\/\/[^\s]*)[\"\']/gi,
    REG_URL: /url\([\'\"]?([\w\.\/\'\"\s\-]+|[a-zA-z]+:\/\/[^\s]*)[\'\"]?\)/gi,
    REG_HREF: /href\=[\"\']([\w\.\/\'\"\s\-]+|[a-zA-z]+:\/\/[^\s]*)[\"\']/gi,
...
function getLinkByMark(fileStr){
    ...
    _.forEach(linkArr, function(item){
        if(item){
            ...
            var _link = {link: _url};
            _link.type = _.lowerCase(_url.substring(_url.lastIndexOf('.')+1, item.length));
            links.push(_link)
        }
    });
    return links;
